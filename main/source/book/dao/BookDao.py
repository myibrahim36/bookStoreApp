...
from main.source.config import BookConfig
from main.source.dao.data_access import DataAccessor
...
class BookDao(DataAccessor):
    ...
    def get_popular_books(self):
        book_list = list()
        try:
            query = ("select books.isbn as isbn, title, authors, publisher, available_copies, price, image_loc, category_id "
                     "from books"
                     " left join rating"
    ...
    def get_all_books(self):
        try:
            query = (
                ...
                "available_copies, price, image_loc "
                "from books")
            book_list = super(BookDao, self).read(query=query)
    ...
    def get_books_by_category(self, catData):
        ...
        try:
            ...
            query = ("select isbn, title, authors, publisher, available_copies, price, image_loc,cc.name as childcategory "
                     "from books"
    ...
    def getBookInfo(self, isbn):
        query = ("select isbn, title, authors, publisher, "
                 "available_copies, price, image_loc,description from books as b"
